...
export default function InitialSetupForm() {
  ...
  const handleStartSetup = async () => {
    ...
    try {
      ...
      setQrCodeDataUrl((json as SetupStartResponse).qrCodeDataUrl ?? "");
      setSecretBase32((json as SetupStartResponse).secretBase32 ?? "");
      setEmail((json as SetupStartResponse).email ?? email);
      setStep(2);
    } catch {
  ...
  const handleConfirmSetup = async () => {
    ...
    try {
      ...
      setMessage("TOTP setup completed");
      router.replace("/setup/security-key");
      router.refresh();
    } catch {
      ...
      >
        <Stack spacing={2}>
          ...
          <Typography variant="body2" color="text.secondary">
            Create the admin account and connect Google Authenticator.
          </Typography>
          ...
          {step === 1 ? (
            ...
            <>
              ...
              >
                {isLoading ? "Finishing..." : "Continue to Security Key Setup"}
              </Button>
            </>
          )}
        </Stack>
